...
if (selected == "Implementation"):
     with st.form("my_form"):
        ...
        male = st.selectbox('Male (Jenis Kelamin [0 =''Perempuan', 1='Laki-Laki''])', options=[0, 1])
        age = st.number_input('Age (Usia Pasien [0 =''Tidak', 1='Ya''])', min_value=0, max_value=100)
        currentSmoker = st.selectbox('currentSmoker (Perokok [0 =''Tidak', 1='Ya''])', options=[0, 1]) 
        cigsPerDay = st.number_input('cigsPerDay (Jumlah Rokok Perhari)',  min_value=0, max_value=100) 
        BPMeds = st.selectbox('BPMeds (Menjalani Pengobatan Tekanan Darah [0 =''Tidak', 1='Ya''])', options=[0, 1]) 
        prevalentStroke = st.selectbox('prevalentStroke (Mengalami Penyakit Stroke [0 =''Tidak', 1='Ya''])', options=[0, 1]) 
        prevalentHyp = st.selectbox('prevalentHyp (Mengalami Hypertensi [0 =''Tidak', 1='Ya''])', options=[0, 1]) 
        diabetes = st.selectbox('diabetes (Menderita Diabetes [0 =''Tidak', 1='Ya''])', options=[0, 1])  

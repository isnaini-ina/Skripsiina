...
if (selected == "Implementation"):
     with st.form("my_form"):
        ...
        male = st.selectbox('Male', options=["0 = Perempuan", "1 = Laki-laki"])
        age = st.number_input('Age (Usia Pasien)', min_value=0, max_value=100)
        currentSmoker = st.selectbox('currentSmoker (Perokok)', options=["0 = Tidak", "1 = Ya"]) 
        cigsPerDay = st.number_input('cigsPerDay (Jumlah Rokok Perhari)',  min_value=0, max_value=100) 
        BPMeds = st.selectbox('BPMeds (Menjalani Pengobatan Tekanan Darah )', options=["0 = Tidak", "1 = Ya"]) 
        prevalentStroke = st.selectbox('prevalentStroke (Mengalami Penyakit Stroke )', options=["0 = Tidak", "1 = Ya"]) 
        prevalentHyp = st.selectbox('prevalentHyp (Mengalami Hypertensi)', options=["0 = Tidak", "1 = Ya"]) 
        diabetes = st.selectbox('diabetes (Menderita Diabetes)', options=["0 = Tidak", "1 = Ya"])  
